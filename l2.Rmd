...
y <- sort((shift-mean(shift))/sd(shift))
p.seq <- seq(0,1,length.out = (length(y)+1))
p.seq2<-p.seq[-length(p.seq)] + diff(p.seq) / 2
x <- qnorm(p.seq2)
points(x,y)
```

```{r}
qqnorm(shift, pch = 1, frame = FALSE)
qqline(shift, col = "steelblue", lwd = 2)
```

```{r}
#manually plot
qqnorm(shift,pch=19,col="gold2") 
abline(a=mean(shift),b=sd(shift),lty=2,col="indianred",lwd=2)
...
library(mnormt)
curve(dmt(x,mean=mmu,S=stmu,df=n+2),col="chocolate2",
      lwd=2,xlab="x",ylab="",xlim=c(-.5,.5))
...
```{r}
BaFa=function(z,rat){sqrt(1/(1+rat))*exp(z^2/(2*(1+1/rat)))}
#rat denotes the ratio tau^2/sigma^2 sqrt(1/(1+rat))*exp(z^2/(2*(1+1/rat)))} BaFa(mean(shift),1)
BaFa(mean(shift),1)
BaFa(mean(shift),10)
```
...
tildepi1=function(sigma, mu){
  exp(-.5*((n*xbar^2+sqar)/sigma+(n+2)*log(sigma))+ dnorm(mu,muhat,tauat,log=T))
}

tildepi2=function(sigma,mu){
  exp(-.5*((n*(xbar-mu)^2+sqar+mu^2)/sigma+(n+3)*log(sigma)+ log(2*pi)))
}
